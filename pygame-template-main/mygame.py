...
ENEMYES_IMG_LIST = ["sanes.png","senes.png","microp.png"]
...
def spawnEnemyes():
    x = rd(0,WIDTH)
    y=rd(0,200)
    enemy = Enemy(f"./images/enemies/{ENEMYES_IMG_LIST[rd(0,len(ENEMYES_IMG_LIST)-1)]}",x=x, y=y,speed =rd(1,3))
...
# |___________________________|
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            isRunning = False
            # если нажали на пробел то создаём пулю и добавляем в список
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1 :
                pos= event.pos
                bullet = player.shoot(pos[0], pos[1])
                bulletList.append(bullet)
